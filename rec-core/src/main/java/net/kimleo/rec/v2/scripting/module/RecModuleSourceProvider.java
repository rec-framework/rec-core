    ...
    private Reader getReader(String moduleId) throws IOException {
        if (moduleId.endsWith(".js")) {
            try {
                ...
                LOGGER.warn(format("Loading user defined js module [%s] failed, trying resource instead", moduleId));
                try {
                    return new InputStreamReader(this.getClass().getClassLoader().getResourceAsStream(moduleId));
                } catch (Exception ex) {
            ...
            try {
                LOGGER.warn(format("Loading user defined js module [%s] failed, trying resource instead", moduleId));
                return new InputStreamReader(this.getClass().getClassLoader().getResourceAsStream(moduleId + ".js"));
            } catch (Exception ex) {
                LOGGER.error(format("Loading resource module [%s] failed, error: %s", moduleId, ex));
            }
        }
